...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a FedBN segmentation model")
    ...
    parser.add_argument("--save_dir", type=str, default="weights_fedbn")
    parser.add_argument("--save_name", type=str, default="")
    parser.add_argument("--log_dir", type=str, default="logs_fedbn")
...
def get_batchnorm_state_keys(model):
    ...
    for module_name, module in model.named_modules():
        ...
        for buffer_name, _ in module.named_buffers(recurse=False):
            bn_keys.add(prefix + buffer_name)
...
def main():
    ...
    bn_state_keys = get_batchnorm_state_keys(global_model)
    client_states = {
    ...
    if args.save_name.strip():
        ...
        save_name = f"FedBN_{args.model_name}_{args.dataset_class}.pth"
    ...
    print(f"BN state keys : {len(bn_state_keys)}")
    ...
    for round_idx in range(1, args.rounds + 1):
        ...
        for center in train_centers:
            ...
            local_model.load_state_dict(
                merge_global_and_local_state(global_state, client_states[center], bn_state_keys)
            )
        ...
        for center in eval_centers:
            if center in client_states:
                ...
                eval_model.load_state_dict(
                    merge_global_and_local_state(global_model.state_dict(), client_states[center], bn_state_keys)
                )
            else:
                eval_model = build_model(args.model_name).to(device)
            ...
            torch.save(
                {
                    ...
                    "bn_state_keys": sorted(bn_state_keys),
                },
                save_path,
            )
            print(f"Saved best personalized global state to: {save_path}")

    print("FedBN training finished.")
